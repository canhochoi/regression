...
class Plotting(PlottingSamplingMixin):
    ...
    def plot_scatter_with_se(self, data_df, frames_dict, metric_name,
                         ...
                         label_fontsize = 12, title_labelsize = 12, annotate_fontsize = 8,
        ...
        if annotate:
            for xi, yi, label in zip(x_mean, y_mean, parts):
                ax.annotate(label, (xi, yi), textcoords="offset points", xytext=(5, 2), fontsize=annotate_fontsize)

        ax.set_xlabel(f"{metric_name} ({name_x})", fontsize = label_fontsize)
        ax.set_ylabel(f"{metric_name} ({name_y})", fontsize = label_fontsize)
        ax.set_title(f"{metric_name}: {name_x} vs {name_y} (mean ± SE across bins)", fontsize = title_labelsize)

        x_kwargs = dict(axis="x", which="both", labelsize=xtick_labelsize)
        y_kwargs = dict(axis="y", which="both", labelsize=ytick_labelsize)
        ax.tick_params(**x_kwargs)
        ax.tick_params(**y_kwargs)
    ...
    def scatter_two_celltypes_panel(self, celltype_x, metrics_by_bin_x,
                                    ...
                                    ncols=2, figsize=None, annotate=True, refline=True,title_fontsize=12):
        """
        ...
        for i, m in enumerate(metric_names):
            ...
            self.scatter_two_celltypes_bins(celltype_x=celltype_x,
                                            ...
                                            annotate=annotate,
                                            refline=refline)
            ax.set_title(f"{m}: {celltype_x} vs {celltype_y}", fontsize = title_fontsize)
    ...
    def scatter_design_matrices(self, metrics_dict, metric_names,
                            ...
                            title_labelsize = 12, annotate_fontsize = 8, 
                            xtick_labelsize = 10,
                            ytick_labelsize = 10):
        """
        ...
        for idx, (metric, (celltype_x, celltype_y)) in enumerate(itertools.product(metric_names, pairs)):
            ...
            self.scatter_two_celltypes_bins(
                ...
                annotate=annotate,
                refline=refline,
                title_labelsize = title_labelsize, 
                annotate_fontsize = annotate_fontsize, 
                xtick_labelsize = xtick_labelsize, ytick_labelsize = ytick_labelsize
            )
            ax.set_title(f"{metric}: {celltype_x} vs {celltype_y}", fontsize=title_labelsize)
    ...
    def scatter_two_celltypes_bins(self, celltype_x, metrics_by_bin_x, celltype_y, metrics_by_bin_y, metric_name, 
                                   ax=None, annotate=True, refline=True, tick_labelsize = 12, title_labelsize = 12, 
                                   annotate_fontsize = 8, xtick_labelsize = 10, ytick_labelsize = 10):
        # better version
    ...
    def plot_test_vs_obs_grid(self, Y_test_df, Y_obs_df, ncols=4, figsize=None,
                          ...
                          label_fontsize = 12, x_tick_labelsize = 10, y_tick_labelsize = 10, title_fontsize = 12):
        """
        ...
        for i, part in enumerate(common_cols):
            ...
            ax.set_title(f"{title_prefix}{part} (r={r_pearson:.3f}, R²={r_sq:.3f})", fontsize=title_fontsize)
            ax.set_xlabel("Predicted proportion", fontsize = label_fontsize)
            ax.set_ylabel("Observed proportion", fontsize = label_fontsize)
            ax.grid(True, alpha=0.2)

            x_kwargs = dict(axis="x", which="both", labelsize=x_tick_labelsize)
            y_kwargs = dict(axis="y", which="both", labelsize=y_tick_labelsize)
            ax.tick_params(**x_kwargs)
            ax.tick_params(**y_kwargs)
    ...
    def plot_box_scatter(self, df_long, metric_label="Correlation",
                         ...
                         label_fontsize = 12, title_fontsize = 12,
                         x_tick_labelsize = 12, y_tick_labelsize = 12
                        ):
        """
        ...
        ax.set_title(f"{metric_label} per cell type across folds", fontsize = title_fontsize)
    ...
    def distribution_across_folds(self, metrics_by_bin, metric_name, label_fontsize = 12, title_fontsize = 12,
                                  x_tick_labelsize = 12, y_tick_labelsize = 12):
        ...
        fig, ax = self.plot_box_scatter(df_metric, metric_label = metric_name, hue="fold", 
                                        ...
                                        x_tick_labelsize = x_tick_labelsize, y_tick_labelsize = y_tick_labelsize)
        return fig, ax
    ...
    def plot_sampling_summary_scatter_panel(self,
        ...
        for i, m in enumerate(metric_names):
            ...
            for j, (ct_x, ct_y) in enumerate(pairs):
                ...
                # Labels and title
                ax.set_xlabel(f"{m} ({ct_x})", fontsize = label_fontsize)
                ax.set_ylabel(f"{m} ({ct_y})", fontsize = label_fontsize)
                ax.set_title(f"{m}: {ct_x} vs {ct_y}", fontsize = title_fontsize)
