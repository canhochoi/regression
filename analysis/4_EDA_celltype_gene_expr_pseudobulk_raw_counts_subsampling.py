...
def subset_adata_per_sample_fast(our_cdata, cell_type, n_cells=100, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    for sample_id, group in sample_groups:
        sample_cells = group.index  # all cells in sample
        ...
        if len(type_cells) > n_cells:
            picked_cells = np.random.choice(type_cells, n_cells, replace=False)
        else:
            picked_cells = type_cells

        # combine: all non-target cells + sampled target cells
        other_cells = sample_cells.difference(type_cells)
        cell_combined_samples.extend(other_cells.tolist())
        cell_combined_samples.extend(picked_cells.tolist())
...
for sampling_id in np.arange(args.n_sampling):
    ...
    if args.cell_type == "HSC_MPP":
        our_cdata = subset_adata_per_sample_fast(our_cdata_org, cell_type=args.cell_type, n_cells=1000, seed=sampling_id)
    else:
        our_cdata = subset_adata_per_sample_fast(our_cdata_org, cell_type=args.cell_type, n_cells=1000, seed=sampling_id)
    ...
    if args.cell_type == "HSC_MPP":
        cells_by_samples = (our_cdata.obs.loc[our_cdata.obs['cell_type'].isin(args.cell_type.split("_"))]
        .groupby('sample_id')
        .apply(lambda g: sorted(g.index))
        .to_dict())
    else:
        cells_by_samples = (our_cdata.obs.loc[our_cdata.obs['cell_type'].isin(normalize_cell_types(args.cell_type))]
        .groupby('sample_id')
        .apply(lambda g: sorted(g.index))
        .to_dict())
    ...
    file_dir = os.path.join(args.path_dir, f"{args.cell_type}/")
    ...
    log_cpm_df.to_csv(os.path.join(file_dir, f"mean_gene_expression_pseudobulk_{args.cell_type}_{args.method}_{sampling_id}.csv"))
    import gc
    gc.collect()
    try:
        import cupy as cp
